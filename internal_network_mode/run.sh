...
if [ $PUBLIC_INTERFACE == $FLAT_INTERFACE ] || [ $MANAGE_INTERFACE == $FLAT_INTERFACE ]; then
    echo "Data interface can't be the same with the public interface or manage interface."
    exit 0
fi
...
# For all nodes
for arg in node_type PUBLIC_INTERFACE PUBLICIP PUBLICIP_MASK PUBLICIP_GATEWAY PUBLIC_NETWORK DNS_SERVER MANAGE_INTERFACE MANAGEIP MANAGEIP_MASK FLAT_INTERFACE FIXED_RANGE VM_NETWORK_SIZE RABBIT_PASSWORD ADMIN_PASSWORD SERVICE_PASSWORD ADMIN_TOKEN SERVICE_TOKEN SERVICE_TENANT_NAME KEYSTONE_USER KEYSTONE_PASS GLANCE_USER GLANCE_PASS NOVA_USER NOVA_PASS
do
    export $arg
done
# For Control node
