...
export SERVICE_ENDPOINT="http://${EXT_HOST_IP}:35357/v2.0"
...
create_endpoint () {
  case $1 in
    ...
    ;;
    identity)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':5000/v2.0' --adminurl 'http://'"$EXT_HOST_IP"':35357/v2.0' --internalurl 'http://'"$HOST_IP"':5000/v2.0'
    ;;
    ec2)
...
for i in compute image object-store identity ec2 network; do
